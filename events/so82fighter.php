...
$link->set_charset("utf8");
    // поиск мероприятий
    $query = 'SELECT id, parent_id, name, place, start_time, end_time, comments, contact, lastUpdated, visibility FROM events WHERE (end_time >= CURRENT_TIMESTAMP AND visibility <= 3) ORDER BY start_time;';
    $rt = mysqli_query($link, $query) or die('Запрос не удался: ');
    while ($line = mysqli_fetch_array($rt, MYSQL_ASSOC)) {
      if (isset($line["parent_id"])) {
        $quer = "SELECT name FROM events where visibility <= 3 AND id=".$line["parent_id"].";";
