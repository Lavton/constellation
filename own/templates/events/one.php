...
?>
      ...
      <div class="row own-row">
        ...
        <div class="col-xs-7 info-str">
          ...
          <div ng-show="vk_export">
            ...
            <br> Это шаблонная концовка новости про мероприятие. Добавьте, что хотите, Скопируйте текст в буффер обмена и вставьте в нужные группы: <a href="https://vk.com/spo_sozvezdie" target="_blank">бойцов</a> или|и <a href="https://vk.com/sozvezdie_school" target="_blank">кандидатов</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="event-edit hidden">
    ...
    <br>
    <li>место:
      <input ng-model="newevent.place" placeholder="место мероприятия" size=50 /> </li>
    <br>
    <li>Дата начала*:
      <input type="date" class="date" ng-model="newevent.start_date"> {{newevent.start_date}}
      <br> Время начала*:
      <input type="text" ng-model="newevent.start_time"> {{newevent.start_time}}</li>
    <li>Дата окончания*:
      <input type="date" class="date" ng-model="newevent.finish_date"> {{newevent.finish_date}}
      <br> Время окончания*:
      <input type="text" ng-model="newevent.finish_time"> {{newevent.finish_time}} </li>
    <br>
    <li ng-hide="event.shift_id">Контактое лицо:
      <input type="text" ng-model="newevent.contact">
    </li>
    <?php if (isset($_SESSION["current_group"]) && ($_SESSION["current_group"] >= COMMAND_STAFF)) { ?>
      <li><strong>Виден для: </strong>
        <input type="number" min="1" max="7" ng-model="newevent.visibility" size="{{(newevent.visibility).length}}" ng-init="newevent.visibility=3" /> ({{window.groups[window.visibilities[newevent.visibility]].rus}})</li>
      <?php } else { ?>
        <li ng-show="event.visibility"><strong>Виден для: </strong>{{event.visibility}} ({{window.groups[window.visibilities[event.visibility]].rus}})</li>
        <?php } ?>
          <li>Описание:
            <br>
            <textarea ng-model="newevent.comments" class="bbcode" cols="20" rows="5"></textarea>
          </li>
          <br>
  ...
  <div class="col-xs-7 info-str" ng-hide="event.shift_id">
    ...
    <div>
      ...
      <input class="vk_input" ng-model="personToAdd">
      <br>
      <ul>
